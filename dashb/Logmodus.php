...
function plotLine(unix,buTag,betr) {// Unix sind die Unix-Timestamps
	...
	function getYPixel(val) {
		...
		orgVal = graph.height - (((graph.height - 0) / (getMaxY()+10)  ) * valTrans) - yPadding; // more or less the original scheme
		return orgVal;
	}
	...
	$(document).ready(function() {
		...
		for(h=0;h<labelTimes.length;h++) {
			c.beginPath();		
			c.moveTo((xPadding+48)+pixelPerUnit*h, graph.height - yPadding-11);
