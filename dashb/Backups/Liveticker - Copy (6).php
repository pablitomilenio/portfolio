...
<img id="colon" src="Ziffern/colon.png">
...
function plotLine(unix,buTag,betr) {// Unix sind die Unix-Timestamps
	...
	$(document).ready(function() {
		...
		for(var i = 0; i < yes.length; i ++) {  
			...
			if(getXPixel(l-1)<(window.innerWidth-$("#percWindow").width())) {
				$("#percWindow").css("top",Math.min(Math.max(getYPixel(yes[i]) - $("#percWindow").height()/2,$("#percWindow").height()/3),screen.height - 350));
				$("#percWindow").css("left",Math.min(window.innerWidth - $("#percWindow").width(),getXPixel(l-1)+20));
			} else {
...
$("#timeDisp").css("left",  window.innerWidth/2-$("#timeDisp").width()/2);
numRuns = 0;
isUpdated = false;
blocked = false;
function rs(){
...
if ((dival - cutOff)<llim) toggleClass("Red");
	...
	if(dival < 100) {
		...
		if (fd >= 8) $("#timeDisp").css("background-color","orange");
		else $("#timeDisp").css("background-color","transparent");
	
		//$("#percWindow").width("218");
	}else {
		$("#timeDisp").css("background-color","orange");
...
<div id="div1" style="display:block">hola</div>
