...
if(window.innerWidth < 1415 || window.innerHeight < 695) {
	smallScreen = true;
}
...
// Draw every 10 % a hline
		for(f=40;f<=100;f+=10) {
			...
			c.beginPath();
			c.moveTo(xPadding,  getYPixel(f-cutOff));
			if(f != 80) c.lineTo(graph.width+30-xPadding,  getYPixel(f-cutOff));
			c.stroke();

		}

		c.font = 'bold 30pt MetaPlusLF';
		c.fillText(80+"% ", xPadding - 5, getYPixel(80-cutOff));		
		c.fillText(85+"% ", xPadding - 5, getYPixel(85-cutOff));
		...
		*/
		...
		function drawTimes () {
			...
			pixelPerUnit = (graph.width - (xPadding)) / labelTimes.length;
