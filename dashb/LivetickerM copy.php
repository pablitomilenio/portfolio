...
while ($row = $res->fetch_assoc()) {
    $stack .= (floatval(str_replace(",", ".", $row['ThisMonth']))*100).";";
...
function plotLine(unix,buTag,betr) {// Unix sind die Unix-Timestamps
	...
	function getYPixel(val) {
		orgVal = graph.height - (((graph.height-80) / getMaxY()) * val) - 45;
		return orgVal;
	}
...
// Draw every 10 % a hline
		...
		var linesArr = [0,intDiff/2,intDiff/4,intDiff/1.35,intDiff];
		f=0;
		while(f<linesArr.length) {
			...
			c.fillText(Math.round((linesArr[f]+parseFloat(minval))*100)/100+"%", xPadding + 8, getYPixel(linesArr[f]-cutOff)); // Here are the Y-Axis labels
		...
		for(var i = 1; i < yes.length; i ++) {
			...
			if (yes[i-1]+minval >= mlim) { //starts green
				if(yes[i]+minval >= mlim) { // end also green
					c.strokeStyle =  lineColor;
					toggleClass("Green");
				} 
				if(yes[i]+minval < mlim) { // ends in yellow
					begReColor(lineColor,mlim);
					c.strokeStyle =  mLineColor;
					toggleClass("Yellow");
				}
				if(yes[i]+minval <= llim) { // ends in red
					begReColor(lineColor,mlim);
			...
			if( (yes[i-1]+minval > llim) && (yes[i-1]+minval < mlim) ) 	{ //starts yellow	
				if(yes[i] +minval<= llim) { // end downards to red
					begReColor(mLineColor,llim);
					c.strokeStyle =  lLineColor;
					toggleClass("Red");
				} if(yes[i]+minval >= mlim) { // end upwards in green
					begReColor(mLineColor,mlim);
					c.strokeStyle =  lineColor;
					toggleClass("Green");
				} 
			}
			if (yes[i-1]+minval <= llim) 	{ // starts red
				if(yes[i]+minval <= llim) { // end also red
					c.strokeStyle =  lLineColor;
					toggleClass("Red");
				} 
				if(yes[i]+minval > llim) { // ends in yellow
					begReColor(lLineColor,llim);
					c.strokeStyle =  mLineColor;
					toggleClass("Yellow");
				}
				if(yes[i]+minval >= mlim) { // ends in green
					begReColor(lLineColor,llim);
		...
		for(var i = 0; i < yes.length; i ++) {  
			...
			if (yes[i]+minval>=mlim) c.fillStyle = lineColor;
			if ((yes[i]+minval<mlim) && (yes[i]+minval>=llim)) c.fillStyle = mLineColor;
			if (yes[i]+minval<llim)  c.fillStyle =  lLineColor;
...
llim = 79.99 - cutOff; // untere Grenze unter der es rot ist
