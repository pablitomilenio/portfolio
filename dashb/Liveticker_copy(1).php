...
function plotLine(unix,buTag,betr) {// Unix sind die Unix-Timestamps
	...
	function getYPixel(val) {
		valTrans = Math.pow(1.0470,val);
		orgVal = graph.height - (((graph.height - 0) / (getMaxY()+7)  ) * valTrans) - 45; // more or less the original scheme
		return orgVal;
	}
	...
	$(document).ready(function() {
		...
		for(h=0;h<labelTimes.length;h++) {
			c.beginPath();		
			c.moveTo((xPadding+48)+pixelPerUnit*h, graph.height - 55);
			c.lineTo((xPadding+48)+pixelPerUnit*h, 43);
			c.stroke();		
...
// Draw every 10 % a hline
		...
		var linesArr = [0,55,70,90,95,100];
		f=0;
		...
		*/
		...
		function drawTimes () {
			...
			for(h=0;h<labelTimes.length;h++) {
				c.fillText(labelTimes[h], (xPadding+93)+pixelPerUnit*h, graph.height+46 - yPadding); // Text of hour breaks
