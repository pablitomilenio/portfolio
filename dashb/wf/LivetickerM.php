...
while ($row = $res->fetch_assoc()) {
    if (floatval(str_replace(",", ".", $row['ThisMonth']))*100 > 0) {
...
function toggleClass(name){
	if (name == 'Green') nameB = 'fuchsia';
	if (name == 'Yellow') nameB = 'fuchsia';
	if (name == 'Red') nameB = 'fuchsia';
...
function plotLine(unix,buTag,betr) {// Unix sind die Unix-Timestamps
	...
	$(document).ready(function() {
		...
		c.lineWidth = 1;
		if(dival < (85-minval) && dival >= (80-minval)) c.strokeStyle = 'black';
...
// Draw every 10 % a hline
		...
		var linesArr = [0,intDiff/2,intDiff/4,intDiff/1.333,intDiff];
		f=0;
		while(f<linesArr.length) {
			if (c.setLineDash) c.setLineDash([2,2]);
			
			if((dival >= 80-minval) && (dival <= 85-minval))	c.strokeStyle = 'black';
			else c.strokeStyle = 'white';
			c.lineWidth = 1;
...
mlim = 84.99 - cutOff; // grenze zwischen gruen und gelb
llim = 79.99 - cutOff; // untere Grenze unter der es rot ist

lineColor = "#404040";
mLineColor = "#404040"; //"#595959";
lLineColor = "#404040"; //"#7F7F7F";
...
function ctl() {
	...
	document.title = "→ "+ ttl +" - (Ɛα) ←";

	if (vals[vals.length-1] < llim) toggleClass('Red');
	if (vals[vals.length-1] < mlim) toggleClass('Yellow');
	else toggleClass('Green');
